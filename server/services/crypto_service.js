...
const initVector = config.crypto_config.initVector;
const algorithm = config.crypto_config.algorithm;
...
const ComputeSignature = async (stringToSign) => {
  let secret = process.env["COMMUNICATION_SERVICES_ACCESSKEY"];
...
const decryptData = (encryptedData) => {
  const decipher = crypto.createDecipheriv(algorithm, ENCRYPTION_KEY, initVector);
