...
const redeemPromotion = async (req, res) => {
  const { reward } = res.locals;
  try {
    const reward_ref = await promotionService.complete_promotion(reward);
    if (reward_ref === null) {
...
const redeemPoints = async (req, res) => {
  try {
    ...
    const points = await promotionService.redeem_points(
      user_id,
      point_value,
      title
    );
...
const fetchTransactionList = async (req, res) => {
  try {
    ...
    const transaction_list = await promotionService.get_transaction_list(
      user_id
    );
